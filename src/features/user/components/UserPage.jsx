...
import UserPageHeader from './sections/UserPageHeader';
...
const UserPage = ({ onBack }) => {
  ...
  const {
    formattedDate,
    goHome
  } = useUserPageHeader(formData.noteDate);
      ...
      <main className="update-form">
        <form id="dailyUpdateForm" onSubmit={handleSubmit}>
          ...
          <CollapsibleSection
            ...
            onToggle={() => setJournalExpanded(prev => !prev)}
          >
                <div className="form-group">
...
◆ Bài học rút ra?`}
          ...
          <CollapsibleSection
            title="Photo Album"
            isExpanded={photoAlbumExpanded}
            onToggle={() => setPhotoAlbumExpanded(prev => !prev)}
          >
                <div className="form-group">
          ...
          <CollapsibleSection
            ...
            onToggle={() => setGalleryExpanded(prev => !prev)}
          >
                <div className="form-group">
          ...
          {/* Daily Quests Update */}
          <CollapsibleSection
            title={`Daily Quests Update (${getAvailableQuestsForDropdown().length + selectedQuestSubmissions.length})`}
            isExpanded={questsExpanded}
            onToggle={() => {
          ...
          {/* Achievements Update */}
          <CollapsibleSection
            title={`Achievements Update (${getAvailableAchievementsForDropdown().length + selectedAchievementSubmissions.length})`}
          ...
          <CollapsibleSection
            title={`Review Submitted (${reviewSubmittedLoading ? 'Loading...' : (getAllQuestSubmissions().length + getAllAchievementSubmissions().length)})`}
            isExpanded={reviewSubmittedExpanded}
            onToggle={() => setReviewSubmittedExpanded(prev => !prev)}
          >
